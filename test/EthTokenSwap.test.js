...
describe("DHT", function () {
  ...
  beforeEach(async function () {
    ...
    const dhtAmount = D18.mul(10000000);
    tx = await this.dht.mint(dhtAmount, overrides);
    await tx.wait(1);

    // 向合约打入token
    const EthTokenSwap = await ethers.getContractFactory("EthTokenSwap");
    this.ethtokenswap = await EthTokenSwap.deploy("0x7465737400000000000000000000000000000000000000000000000000000000", "eth_token_swap", 1000, 1616599208, other3.address, this.dht.address);
