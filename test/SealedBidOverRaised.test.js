...
describe("FixedSwap", function () {
  ...
  it("over-raised", async function() {
    ...
    tx = await this.usdt.transfer(other1.address, D8.mul(120), overrides);
    await tx.wait(1);
    await this.usdt.connect(other1).approve(this.sealedbidswap.address, D8.mul(120));
    await tx.wait(1);
    tx = await this.sealedbidswap.connect(other1).purchase(D8.mul(120), overrides);
    await tx.wait(1);

    tx = await this.usdt.transfer(other2.address, D8.mul(120), overrides);
    await tx.wait(1);
    await this.usdt.connect(other2).approve(this.sealedbidswap.address, D8.mul(120));
    await tx.wait(1);
    tx = await this.sealedbidswap.connect(other2).purchase(D8.mul(120), overrides);
    await tx.wait(1);

    tx = await this.usdt.transfer(other3.address, D8.mul(120), overrides);
    await tx.wait(1);
    await this.usdt.connect(other3).approve(this.sealedbidswap.address, D8.mul(120));
    await tx.wait(1);
    tx = await this.sealedbidswap.connect(other3).purchase(D8.mul(120), overrides);
    await tx.wait(1);
    ...
    await expect(this.sealedbidswap.connect(other3).settle(overrides))
      ...
      expect(await this.usdt.balanceOf(other3.address)).to.eq(2000040000);
      expect(await this.xyz.balanceOf(other3.address)).to.eq(33333200000000000000);
  })
});
