...
describe("FixedSwap", function () {
  ...
  beforeEach(async function () {
    ...
    // 向合约打入xyz
    tx = await this.xyz.transfer(this.sealedbidswap.address, D18.mul(120000), overrides);
    await tx.wait(1);

    tx = await this.usdt.transfer(other1.address, D8.mul(5000), overrides);
    await tx.wait(1);
    await this.usdt.connect(other1).approve(this.sealedbidswap.address, D8.mul(5000));
    await tx.wait(1);
    tx = await this.sealedbidswap.connect(other1).purchase(D8.mul(1000), overrides);
    await tx.wait(1);
