...
describe("FixedSwap", function () {
  ...
  beforeEach(async function () {
    ...
    this.fixedswapfactory = await FixedSwapFactory.deploy(
      deployer.address,
      overrides
    );
    console.log("factory address:", this.fixedswapfactory.address)
    tx = await this.fixedswapfactory.instantiate(
      345,
      "fixed_swap",
      deployer.address,
      this.usdt.address,
    ...
    this.fixedswap_addr = await this.fixedswapfactory.getAuction(345);
    console.log("auction pair address:", this.fixedswap_addr)
    ...
    tx = await FixedSwap.attach(this.fixedswap_addr).setQuota(other1.address, D8.mul(100));
    await tx.wait(1);

    // 向合约打入xyz
    tx = await this.xyz.transfer(this.fixedswap_addr, D18.mul(120000), overrides);
    await tx.wait(1);

    tx = await this.usdt.transfer(other1.address, D8.mul(100), overrides);
    await tx.wait(1);
    await this.usdt.connect(other1).approve(this.fixedswap_addr, D8.mul(100));
    await tx.wait(1);
    tx = await FixedSwap.attach(this.fixedswap_addr).connect(other1).offer(D8.mul(10), overrides);
    await tx.wait(1);
    ...
    tx = await FixedSwap.attach(this.fixedswap_addr).connect(other1).claim(overrides);
    await tx.wait(1);

    expect(await this.xyz.balanceOf(other1.address)).to.eq(D18.mul(5000));
    expect(await this.xyz.balanceOf(this.fixedswap_addr)).to.eq(D18.mul(115000));
  });
