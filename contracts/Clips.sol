...
contract Clips is Context, IERC20, ReentrancyGuard {
    ...
    modifier owner_only {
        require(_msgSender() == _owner, "ICA: Owner only");
        _;
    }
    ...
    function setSoftCap(uint256 n) public owner_only {
        require(n <= _cap, "ICA: softcap overflows cap");
        require(
            n >= _totalSupply,
            "ICA: softcap must be higher than total supply"
        );
        _softcap = n;
    }
    ...
    function setMintAccount(address account) public owner_only {
    ...
    function getPriorVotes(address account, uint256 blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "ICA: not yet determined");
